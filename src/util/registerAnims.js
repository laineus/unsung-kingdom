...
export default scene => {
  assets.spritesheet.forEach(args => {
    if (args[2].endFrame === 12) {
      ...
      scene.anims.create({ key: `${args[0]}_walk_front`, frames: scene.anims.generateFrameNumbers(args[0], { start: 0, end: 2 }), repeat: -1, frameRate: 5 })
      scene.anims.create({ key: `${args[0]}_walk_back`, frames: scene.anims.generateFrameNumbers(args[0], { start: 3, end: 5 }), repeat: -1, frameRate: 5 })
    }
  })
}
