...
export default class BattlerSummary extends Phaser.GameObjects.Container {
  constructor (scene, x, y, chara) {
    ...
    const text = scene.add.text(-23, 4, chara.name, { fontSize: 12, fontStyle: 'bold', fontFamily: config.FONTS.UI }).setOrigin(0, 1)
    this.lv = this.scene.add.text(-12 + chara.name.length * 4, -12, '', { fill: config.COLORS.theme.toColorString, fontSize: 11, fontStyle: 'bold', fontFamily: config.FONTS.UI })
