...
export default class SpeachBubble extends Phaser.GameObjects.Container {
  constructor (scene, x, y, name, text) {
    ...
    this.name = scene.add.text(-50, -65, name, { fill: config.COLORS.white.toColorString, fontSize: 17,  fontStyle: 'bold', stroke: config.COLORS.dark.toColorString, strokeThickness: 4 }).setPadding(0, 2, 0, 0)
    this.text = scene.add.text(54, -6, '', { fill: config.COLORS.white.toColorString, fontSize: 14, lineSpacing: 5 }).setOrigin(0.5, 0.5).setPadding(0, 2, 0, 0)
    this.setText(text)
