...
export default class SpeachBubble extends Phaser.GameObjects.Container {
  constructor (scene, x, y, name, imageKey, text, position = -1) {
    ...
    this.image = scene.add.sprite(left ? -120 : 102, -81, imageKey).setOrigin(0.5, 0).setScale(0.66, 0.66).setMask(mask)
    this.name = scene.add.text(left ? -70 : -140, left ? -65 : -72, name, { fill: config.COLORS.white.toColorString, fontSize: 17,  fontStyle: 'bold', stroke: config.COLORS.dark.toColorString, strokeThickness: 4 }).setPadding(0, 2, 0, 0)
    this.text = scene.add.text(left ? 44 : -44, -6, '', { fill: config.COLORS.white.toColorString, fontSize: 14, lineSpacing: 5 }).setOrigin(0.5, 0.5).setPadding(0, 2, 0, 0)
    this.setText(text)
