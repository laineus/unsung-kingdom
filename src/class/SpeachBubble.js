...
export default class SpeachBubble extends Phaser.GameObjects.Container {
  constructor (scene, x, y, name, imageKey, text, position = -1) {
    ...
    this.text = scene.add.text(left ? 44 : -44, -6, '', { fill: config.COLORS.white.toColorString, fontSize: 14, fontFamily: config.FONTS.TEXT, lineSpacing: 4.5, stroke: config.COLORS.dark.toColorString, strokeThickness: 2, wordWrap: { width: 200, useAdvancedWrap: true } }).setOrigin(0.5, 0.5).setPadding(0, 2, 0, 0)
    this.setText(text)
  ...
  setText (text) {
    this.text.text = text
    this.text.setLineSpacing(this.text.height < 82 ? 4.5 : 0)
  }
}
