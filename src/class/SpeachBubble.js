...
export default class SpeachBubble extends Phaser.GameObjects.Container {
  constructor (scene, x, y, name, imageKey, text, position = -1) {
    ...
    this.name = scene.add.text(left ? -70 : -140, left ? -65 : -72, name, { fill: config.COLORS.white.toColorString, fontSize: 16, fontFamily: config.FONTS.TEXT, fontStyle: 'bold', stroke: config.COLORS.dark.toColorString, strokeThickness: 2 }).setPadding(0, 2, 0, 0)
