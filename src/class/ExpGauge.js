...
export default class ExpGauge extends Phaser.GameObjects.Container {
  constructor (scene, x, y, width, lv, exp) {
    ...
    this.bar = scene.add.rectangle(-width.half, -1, width - 11, 2, config.COLORS.soy).setOrigin(0, 1).setScale(this.barScale, 1)
    this.arrow = scene.add.polygon(width.half -1, 4, [[0, -5], [0, 0], [10, 0]], config.COLORS.soy).setOrigin(1, 1)
