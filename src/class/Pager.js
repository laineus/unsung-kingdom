...
export default class Pager extends Phaser.GameObjects.Container {
  ...
  set (prev = true, next = true) {
    ...
    if (prev) {
      this.pagerPrevious = this.scene.add.container(50, 0).setSize(100, 30).setInteractive().on('pointerdown', () => this.emit('prev'))
    ...
    if (next) {
      this.pagerNext = this.scene.add.container(this.width - 50, 0).setSize(100, 30).setInteractive().on('pointerdown', () => this.emit('next'))
