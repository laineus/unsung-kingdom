...
export default class Button extends Phaser.GameObjects.Container {
  constructor (scene, x, y, text, width = 120, height = 80, { align = 'center', fontSize = 16 } = {}) {
    ...
    this.text = scene.add.text(0, 0, text, { align, fontSize, fontFamily }).setOrigin(alignLeft ? 0 : 0.5, 0.5).setPadding(0, 2, 0, 0)
    width = Math.max(this.text.width + 40, width)
    this.text.setPosition(alignLeft ? -width.half + 20 : 0, -1)
